 ...
 *
 */
public class MicroLoansGrantedComputer extends AbstractMicroComputer implements
	...
	public Map<Long, Double> computeVariables(MacroSimulation sim) {
		...
		for (Agent i:lenders.getAgents()){
			double loansGranted=0;
			MacroAgent lender= (MacroAgent) i;
			if (!lender.isDead()){
			List<Item> loans=lender.getItemsStockMatrix(true, loansId);
			for(Item loan:loans){
				if(loan.getAge()==0) {
					loansGranted+=loan.getValue();
				}
			}	
			
			result.put(lender.getAgentId(), loansGranted);
			}
			else{
