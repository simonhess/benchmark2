...
public class AdaptiveMarkupOnAdvancesRateProfitGrowth extends AbstractStrategy implements
InterestRateStrategy {
	 ...
	 * 
	 * This strategy computes the banks interest rate on loans based on the growth rate of profits. If the growth rate of the profits from the last period
	 * is greater than the growth rate of the period before that the bank computes its new markup by adding a random amount to the average markup of all banks.
...
//		threshold=tot/n;
	...
	double profitGrLastPeriod = lender.getPassedValue(StaticValues.LAG_PROFITAFTERTAX, 1)/lender.getPassedValue(StaticValues.LAG_PROFITAFTERTAX, 2);
	double profitGrPenultimatePeriod = lender.getPassedValue(StaticValues.LAG_PROFITAFTERTAX, 2)/lender.getPassedValue(StaticValues.LAG_PROFITAFTERTAX, 3);
	
	if(profitGrLastPeriod>profitGrPenultimatePeriod) {
	markup=avMarkup+adaptiveParameter*distribution.nextDouble();
}else if(profitGrLastPeriod<profitGrPenultimatePeriod){
	markup=avMarkup-adaptiveParameter*distribution.nextDouble();
}
