...
import benchmark2.expectations.AdaptiveExpectationWithDelta;
...
public class AdaptiveMarkUpOnACTargetInventories extends AbstractStrategy implements
MarkupPricingStrategy {
	 ...
	 * inventories and the target ones with the average price of all firms as base. If the inventories are lower than the target ones, 
	 * which implies an increased demand, the firm will chose a price above the average price. If the inventories are greater than the target
	 *  ones, which implies an decreased demand, the firm will chose a price above the average price.
	 */
	@Override
	public double computePrice() {
		...
		AdaptiveExpectationTargetInventories salesExp = (AdaptiveExpectationTargetInventories) seller.getExpectation(StaticValues.EXPECTATIONS_REALSALES);
		...
		Government gov = (Government) macroPop.getPopulation(StaticValues.GOVERNMENT_ID).getAgentList().get(0);
		Population pop = null;
		if(agent instanceof ConsumptionFirm) {
